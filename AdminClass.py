...
class Admin:
    ...
    def add_book(self, mycursor, ISBN, Title, author_name, author_surname, publisher, published_year, pages, language,
                 book_type, location, section, genre, employee_id, date, Price, status_comment="NULL",
                 translator="NULL", Title_untranslated="NULL", translated_from="NULL", edition="NULL",
                 number_of_copies=1):
        # queries
        add_book_entries = add_to_Book_entries(ISBN=ISBN, status_comment=status_comment)
        print(add_book_entries)
        add_book = add_to_books(ISBN=ISBN, Title=Title, publisher=publisher, published_year=published_year, pages=pages,
                                language=language, edition=edition, book_type=book_type, location=location,
                                section=section, genre=genre)
        print(add_book)
        ...
        # if book not yet in database adds to books, authors, if_translated
        ...
        for l1st in book_id_list:
            for book_id in l1st:
                ...
                mycursor.execute(
                    add_to_Transactions(book_id=book_id[0], employee_id=employee_id, date=date, Price=Price))
...
admin.add_book(mycursor=mycursor, ISBN=9780590353403, Title="Harry Potter and the Sorcerers Stone",
               author_name="Joanne", author_surname="Rowling", publisher="Scholastic Inc", published_year=2003,
               pages="309", language="English (USA)", book_type="Hardcover", location="7", section="7", genre="Fiction",
               employee_id=2, date="2022-06-02", Price=1000, translator="Joanne Rowling",
